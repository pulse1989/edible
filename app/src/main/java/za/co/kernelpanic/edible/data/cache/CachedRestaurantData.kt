package za.co.kernelpanic.edible.data.cache

import androidx.room.*

@Entity(tableName = "restaurants")
data class CachedRestaurantProfile(
    @PrimaryKey @ColumnInfo(name = "restaurant_id") val restaurantId: Long? = 0,
    @ColumnInfo(name = "name") val name: String? = "",
    @ColumnInfo(name = "status") val status: String? = "",
    @ColumnInfo(name = "favourite") val isFavourite: Boolean,
    @Embedded val profile: RestaurantProperties? = null
)
